...
@section('contenido')
    <div class="container mx-auto md:flex">
        <div class="md:w-1/2 p-5">
            <img src="{{ asset('uploads') . '/' . $post->imagen }}" alt="Post de {{ $post->titulo }}">
        ...
        <div class="md:w-1/2 p-5">
            <div class="shadow bg-white p-5 mb-5">
                @auth
                    ...
                    @if (session('mensaje'))
                        <div class="bg-green-500 p-2 rounded-lg ,b-6 text-white text-center uppercase font-bold">
                            {{ session('mensaje') }}
                        </div>
                    @endif
                    ...
                    @if ($post->comentarios->count())
                        @foreach ($post->comentarios as $comentario )
                            <div class="p-5 border-gray-300 border-b">
                                <a href="{{ route('post.index', $comentario->user->username) }}" class="font-bold">
